...
int main(int argc, char *argv[])
{
	...
	if(argc != 4)
    {
            printf("invalid args.\nwiilz77 input output [-pack OR -unpack]");
            goto err;
    }
    if(strcmp(argv[3], "-pack") == 0)
        mode = 1;
    else
        mode = 0;
	...
	filesz = filesize(infile);
	
	buffer = malloc(filesz);
	if(buffer == NULL) 
	{
		printf("Buffer error.\n"); 
		goto err;
    ...
    u32 lz77tag = be32(0x4C5A3737);
    stat(argv[1], &finfo);
    
    if(mode == 0)
    {
	    if(memcmp(&lz77tag, buffer, 4) == 0)
	        Decode();
   	    else
		    fwrite(buffer, 1, filesz, outfile);
    }
    else
        Encode();
